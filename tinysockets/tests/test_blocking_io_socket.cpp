...
TEST(TestBlockingIOSocket, test_minimal_interleaved_two_packets) {
    // 1) Server setup
    const ccoip_socket_address_t listen_address = create_ipv4_address(127, 0, 0, 1, 48148);
    tinysockets::ServerSocket server_socket(listen_address);
    ...
    ASSERT_TRUE(server_socket.runAsync());

    // 2) Client setup
    tinysockets::BlockingIOSocket client_socket(listen_address);
    ASSERT_TRUE(client_socket.establishConnection());

    // 3) Send 2 packets from client -> server
    {
        DummyPacket dp;
        dp.payload = {0x11};
        ASSERT_TRUE(client_socket.sendPacket(dp));

        AnotherDummyPacket ap;
        ap.data = {0x22};
        ASSERT_TRUE(client_socket.sendPacket(ap));
    }
    ...
    ASSERT_EQ(inbound_count.load(), 2) << "Server didn't receive the 2 packets we sent.";
    ...
    std::thread tA([&] {
        for (int i = 0; i < expected_dummy_count; ++i) {
            ...
            ASSERT_TRUE(result.has_value()) << "Failed to receive the next DummyPacket";
    ...
    std::thread tB([&] {
        for (int i = 0; i < expected_another_count; ++i) {
            ...
            ASSERT_TRUE(result.has_value()) << "Failed to receive the next AnotherDummyPacket";
...
TEST(TestBlockingIOSocket, test_interleaved_packet_types) {
    ...
    std::thread tA([&] {
        for (int i = 0; i < expected_from_server_A; ++i) {
            ...
            ASSERT_TRUE(result.has_value()); // We expect them all eventually
            ++client_received_A;
        }
    });
