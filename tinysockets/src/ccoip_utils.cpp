...
int convert_to_sockaddr_ipv6(const ccoip_socket_address_t &ccoip_addr, sockaddr_in6 *sock_addr_out) {
    *sock_addr_out = {};
    if (ccoip_addr.inet.protocol == inetIPv6) {
        ...
        for (int i = 0; i < 16; i++) {
            sock_addr_out->sin6_addr.s6_addr[i] = ccoip_addr.inet.ipv6.data[i];
        }
...
int convert_from_sockaddr(const sockaddr *sock_addr, ccoip_socket_address_t *ccoip_addr) {
    if (sock_addr->sa_family == AF_INET) {
        ...
        ccoip_addr->inet.ipv4.data[0] = addr_in->sin_addr.s_addr & 0xFF;
        ccoip_addr->inet.ipv4.data[1] = (addr_in->sin_addr.s_addr >> 8) & 0xFF;
        ccoip_addr->inet.ipv4.data[2] = (addr_in->sin_addr.s_addr >> 16) & 0xFF;
        ccoip_addr->inet.ipv4.data[3] = (addr_in->sin_addr.s_addr >> 24) & 0xFF;
        ...
        for (int i = 0; i < 16; i++) {
            ccoip_addr->inet.ipv6.data[i] = addr_in6->sin6_addr.s6_addr[i];
        }
    } else [[unlikely]] {
        return -1; // Unsupported protocol
    }
