...
static void configure_socket_fd(const int socket_fd) {
    ...
    constexpr int buffer_size = 1 << 20; // 1 MB
...
bool tinysockets::BlockingIOSocket::sendTlvPacket(const ccoip::packetId_t packet_id,
    ...
    if (const ssize_t i = send(socket_fd, tlv_buffer.data(), tlv_buffer.size(), MSG_NOSIGNAL); i == -1) {
...
ccoip::packetId_t tinysockets::BlockingIOSocket::receivePacketType() const {
    ...
    if (const ssize_t i = recv(socket_fd, &packet_id, sizeof(packet_id), 0); i == -1) {
        const std::string error_message = std::strerror(errno);
...
size_t tinysockets::BlockingIOSocket::receivePacketLength() const {
    size_t length;
    if (const ssize_t i = recv(socket_fd, &length, sizeof(length), 0); i == -1) {
        const std::string error_message = std::strerror(errno);
...
bool tinysockets::BlockingIOSocket::receivePacketData(std::span<uint8_t> &dst) const {
    if (const ssize_t i = recv(socket_fd, dst.data(), dst.size_bytes(), 0); i == -1) {
        const std::string error_message = std::strerror(errno);
