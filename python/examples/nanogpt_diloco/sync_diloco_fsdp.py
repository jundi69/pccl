...
def all_reduce_multiple_with_retry(communicator: Communicator,
    ...
    """
    world_size = communicator.get_attribute(Attribute.LOCAL_WORLD_SIZE)
...
def main():
    ...
    while True:
        ...
        with profiler.session("pccl::update_topology"):
            if local_iter_num > 1 or global_world_size < fsdp_world_size or local_world_size == 1:
                while True:
                    try:
        ...
        if global_world_size < fsdp_world_size:
            print("Not all FSDP workers have joined...")
            time.sleep(1)
            continue
